...
def entry():
    current_round = 1
    for sender, packet in unpacked:
        pt = type(packet)
        if pt == BasicInfo:
            pass
        elif pt == GameDead:
            str_attacker = muid_to_name(packet.attacker)
            str_victim = muid_to_name(packet.victim)
